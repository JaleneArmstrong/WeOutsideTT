export const MAXI_STOPS = [
  ...
  { name: "Mayaro", lat: 10.3, lon: -61.0 },
  { name: "Siparia", lat: 10.1333, lon: -61.5 },
  { name: "La Romaine", lat: 10.2833, lon: -61.5167 },
  { name: "Curepe", lat: 10.65, lon: -61.4167 },
...
export function findMaxiRouteByProximity(
  ...
  const yellowBandStops = ["Diego Martin", "Petit Valley", "Chaguaramas"];

  if (yellowBandStops.includes(nearestToEvent.stop.name)) {
    const HUB = "Port of Spain";

    for (const maxiRoute of MAXI_ROUTES) {
      const hasUserStop = maxiRoute.stops.includes(nearestToUser.stop.name);
      const hasHub = maxiRoute.stops.includes(HUB);

      if (hasUserStop && hasHub) {
        return {
          route: maxiRoute,
          fromStop: nearestToUser.stop.name,
          toStop: HUB,
          ...
          distanceFromEnd: calculateDistance(
            MAXI_STOPS.find((s) => s.name === HUB)!.lat,
            MAXI_STOPS.find((s) => s.name === HUB)!.lon,
            userLat,
            userLon,
          ),
        };
